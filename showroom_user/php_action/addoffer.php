...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	 ...
	 $mdid = test_input($_POST["txtbid"]);
     $sql = "SELECT * from offers where mdid=$mdid AND status=1";
	 $query = $conn->query($sql);
	 $count = $query->num_rows;
      if ($count >0){
        $error="Your Offer is already Running for this Model!";
        $show="display:show;";
        $alert="alert alert-danger";
		header("Location:../addoffer.php?error=$error&show=$show&alert=$alert");
		exit();
      }
...
function upload($uidsend) {
	...
	$msg=null;
      ...
      $description = test_input($_POST["txtodesc"]);     
     include('../lock.php');
     $uid=$user_id;
     $status=1;    
            ...
            if( $_FILES['userfile']['size'] < $maxsize) {  
              ...
              //if(strpos(mime_content_type($_FILES['userfile']['tmp_name']),"image")===0) {
                ...
                if(strpos(finfo_file($finfo, $_FILES['userfile']['tmp_name']),"image")===0) {    
					...
					move_uploaded_file($_FILES["userfile"]["tmp_name"],"../uploads/offers/".$newfilename);
					...
					$imgpath="uploads/offers/".$newfilename;
					
                    // our sql query
					$sql = "INSERT INTO offers (datefrom, dateto, description, imgpath, regdate, status, mdid)VALUES ('$datefrom', '$dateto', '$description', '$imgpath', @now := now(), '$status', $mdid)";                   
					include('../conn.php');
                    // insert the image
                    if($conn->query($sql)===TRUE){
                    //mysql_query($sql) or die("Error in Query: " . mysql_error());
                    $msg='<p>Offer Is Updated successfully !</p>';
                    }
